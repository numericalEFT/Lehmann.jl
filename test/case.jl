...
function plasmon(isFermi, symmetry, Grid, Euv, β; IsMatFreq = false)
    # function Sw(n, β)
    if isFermi && symmetry == :pha
        if IsMatFreq
            return @. 1 / (1 + (π * (2 * Grid + 1) / β)^2)
        else
            return @. (exp(-Grid) - exp(-(β - Grid))) / 2 / (1 + exp(-β))
        end
    elseif isFermi == false && symmetry == :ph
        if IsMatFreq
            return @. 1 / (1 + (2π * Grid / β)^2)
        else
            return @. (exp(-Grid) + exp(-(β - Grid))) / 2 / (1 - exp(-β))
        end
    else
