...
function MultiPole(dlr, grid, type)
    ...
    poles = [-Euv, -0.2 * Euv,0.0, 0.2 * Euv, Euv]
...
for lambda in Λ
    for err in rtol
        ...
        dlr10 =  DLRGrid(10.0, lambda, 10.0^err, true, :sym, rebuild = false, algorithm = algorithm, folder = folder, verbose = true)
        dlrfile = "basis.dat"
        grid = readdlm(dlrfile,'\n')
        dlrfile = "basis_n.dat"
        grid_n = readdlm(dlrfile,'\n')
        dlrfile = "basis_τ.dat"
        grid_τ = readdlm(dlrfile,'\n')
        #println("$(dlr.τ)\n")
        #println("$(grid*lambda)\n")
        print(dlr.ω)
        # dlr.ω = grid[:,1]./lambda
        # dlr.τ = (grid_τ*lambda)[:,1]
        # dlr.n = grid_n[:,1]
        #dlr.n = grid_n10[:,1]
        ...
        Gdlr = copy(Gdlr0)
        Gfreq = copy(Gfreq0)
        Gτ_compare = matfreq2tau(dlr_none, Gfreq, dlr_none.τ, dlr_none.n)
        #dlrcoeff =  tau2dlr(dlr_none,dlr.τ,Gdlr)
        
        Gfreq_compare = tau2matfreq(dlr_none, Gdlr, dlr_none.n, dlr_none.τ)

        print("τ->ω max error from DLR:$(Rtol(Gfreq, Gfreq_compare)) $(findmax(abs.(Gfreq-Gfreq_compare)))\n")
        print("ω->τ max error from DLR:$(Rtol(Gdlr, Gτ_compare)) $(findmax(abs.(Gdlr-Gτ_compare)))\n")

        #Gdlr0 = MultiPole(dlr_none, dlr_none.τ, :τ)
        #Gfreq0 = MultiPole(dlr_none,dlr_none.n, :n)
        Gτsample = tau2tau(dlr_none, Gdlr0, dlr10.τ, dlr_none.τ)
        Gnsample = matfreq2matfreq(dlr_none, Gfreq0, dlr10.n, dlr_none.n)
        ...
        # Gτsample = bare_G(dlr10, E, :τ)
        # Gnsample = bare_G(dlr10, E, :n)
        ...
        #     end
        # end
        Gtau_compare = tau2tau(dlr, Gdlr, dlr.τ, dlr.τ)
        
        print("τ->τ max error from DLR: $(findmax(abs.(Gdlr-Gtau_compare)))\n")
        ...
        print("τ->ω max error from SDLR:$(Rtol(Gfreq, Gfreq_compare)) $(findmax(abs.(Gfreq-Gfreq_compare)))\n")
        ...
        print("ω->τ max error from SDLR:$(Rtol(Gdlr, Gτ_compare)) $(findmax(abs.(Gdlr-Gτ_compare)))\n")

        Gfreq_compare = tau2matfreq(dlr, Gdlr, dlr10.n, dlr.τ)
        print("τ->ω max error from SDLR: $(Rtol(Gnsample, Gfreq_compare)) $(findmax(abs.(Gnsample-Gfreq_compare)))\n")
        Gτ_compare = matfreq2tau(dlr, Gfreq, dlr10.τ, dlr.n)
        print("ω->τ max error from SDLR: $(Rtol(Gτsample, Gτ_compare)) $(findmax(abs.(Gτsample-Gτ_compare)))\n")
